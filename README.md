...
Payment token is $POST. One mint config for the quiet CA slot and for payment matching / `amountRaw`: `VITE_TOKEN_MINT` / `TOKEN_MINT` (empty by default — hide the CA slot, do not render a placeholder, do not match a mint). Amount is `100000` (`VITE_TOKEN_AMOUNT` / `TOKEN_AMOUNT`). Pay address / treasury: `NBQhuKpHq4M6wmGmgAhZKt4yCJ1JqxY7h8Cf3SM2mMQ` (`VITE_TREASURY_ADDRESS` / `TREASURY_ADDRESS`). Public key only. Do not commit secrets. Do not store a private key. Set a new mint in Netlify env (or `pay/types.ts` `DEFAULT_TOKEN_MINT`) — no other code hunt.
