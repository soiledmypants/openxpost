...
export const GREETING =
  "Draft the post. I check the rules. Get a unique ROOTS amount and send exactly that to the receive wallet. Those tokens are burned after they land. The tweet link comes back on this site, never in the tweet.";
...
export function reviewDraft(draft: string): string {
  ...
  return `This can go up. ${remaining} characters left. Get a unique amount, then send that exact ROOTS amount. It will be burned.`;
}
...
export function answer(question: string, draft: string): string {
  ...
  if (/(how|what).*(pay|token|amount|invoice)/.test(q) || q.includes("wallet")) {
    return "No wallet connect. Get a unique ROOTS amount, copy it, and send exactly that to the receive wallet. Those tokens are burned after they land.";
  }
  if (/(burn|flywheel)/.test(q)) {
    return "After the unique ROOTS amount lands, Pay burns it. They are not kept. That is the flywheel.";
  }
